...
generate_date_msg_id(RightPart, DateHeaderValue, MessageId, !RS, !IO) :-
    ...
    DateStr = string.format("%s, %d %s %d %02d:%02d:%02d %+03d%02d",
        [s(WdayName), i(Day), s(MonthName), i(Year), i(Hour), i(Min), i(Sec),
        i(TzHour), i(TzMin)]),
    DateHeaderValue = header_value(DateStr),

    % This mimics the Message-ID format generated by Mutt (pre-2.0) but the
    % sequence char and pid are replaced by random values, for ~54.5 random
    % bits. Arguably, we should switch to a different format, with a larger
    % alphabet.
    generate_uint64(R, !RS),
    C = char.det_from_int(65 + cast_to_int(R mod 26_u64)),
    X = (R >> 5) mod 1_000_000_000_000_000_u64,
    MessageIdStr = string.format("%d%02d%02d%02d%02d%02d.G%c%s@%s",
        [i(Year), i(Month), i(Day), i(Hour), i(Min), i(Sec),
        c(C), s(uint64_to_string(X)), s(RightPart)]),
    MessageId = message_id(MessageIdStr).
